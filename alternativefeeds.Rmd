...
growth_data <- 
  ...
  nest() %>% 
  mutate(Relative_FCR = purrr::map(data, relative)) %>% 
...
ggplot(data = fishmeal, aes(x=Replacement, y=Relative_FCR))+geom_line(aes(colour=Feed_general, group=Study_id))+geom_point()+
...
mod.1 <- lmer(Relative~ Replacement + (1|Amino_supp) +(1|Fish_scientific), data=carps)
