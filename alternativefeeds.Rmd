...
ggplot(data = fishmeal, aes(x=Replacement, y=Relative_FCR))+geom_line(aes(colour=Feed_general, group=Study_id, alpha=Year))+geom_point(aes(alpha=Year))+
  facet_grid(rows = vars(Feed_general), cols= vars(Fish_group), scales="free")+
  #scale_y_continuous(limits = c(0,80))+
  ggsave("growth_fishmeal.tiff", device="tiff", dpi=300, width=11, height=7)

  ggplot(data = fish_oil, aes(x=Replacement, y=Relative_FCR))+geom_point(alpha=.7,(aes(colour=Feed_general)))+geom_line(aes(colour=Feed_general, group=Study_id))+
  facet_grid(rows = vars(Feed_general), cols= vars(Fish_group), scales="free")+
