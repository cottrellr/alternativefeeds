...
a_test <- bind_rows(lapply(1:dim(na.exclude(fed_aqua_proj))[1], function(target_row_id){
  ...
  temp_df <- bind_rows(lapply(1:500, function(rep_id){
    ...
    l <-  case_when(i<j ~ h-j,
                    T ~ h+k) #total forage fish required - so when fo demand is less than that produced from fishmeal consumed, the total forage fish equivalents are discounted by the weight of fish oil available for other uses (i.e. traded)
    ...
    ee <- (cc-dd)/0.05
    ff <- bb+ee #case_when(ee>=0 ~ bb+ee, ee<0 ~ bb)
...
ggarrange(production_scenarios, consumption_scenarios, nrow = 2, ncol=1, align = "hv")+
  ggsave("Figure 1 - production and consumption.tiff", device="tiff", dpi=600, width = 18, height=14, units="cm")+
  ggsave("Figure 1 - production and consumption.pdf", device="pdf", dpi=600, width = 18, height=14, units="cm")
...
(supply_demand <- 
  ...
  scale_y_continuous(limits=c(0,4e+7), labels=c(0,10,20,30,40), expand=c(0,0))+
  theme_pubr()+
...
consumption <- 
  ...
  annotate("text", x=2.5, y=39.5e+6, label="Aquaculture consumption", size=2.8)
...
ggarrange(supply_demand, consumption, widths = c(1,0.7), nrow=1, ncol=2)+
  annotate("text", x=.665,y=.76, label="Average supply 1980-2013", size=2.5)+
  annotate("text", x=.58,y=.605, label="Average animal feed use\n1980-2013", size=2.5, col= "black", hjust=0)+
...
a_test <- bind_rows(lapply(1:dim(na.exclude(fed_aqua_proj))[1], function(target_row_id){
  ...
  temp_df <- bind_rows(lapply(1:500, function(rep_id){
    ...
    j <- h*0.05   # amount of fish oil coming from the fm biomass
    k <- (i-j)/0.05 # difference in the amount of fish oil still needed (surplus) converted into forage fish biomass 
    l <- h+k  #total forage fish required (which includes when k is less than zero so consumption is not penalised for fish oil that may be provided elsewhere)
...
```{r}
 ...
 baseline_2015 <- 
  ...
  select(Country, Species, Taxa_adj, rep_id, consumption_2015) %>% 
  group_by(Taxa_adj, rep_id) %>% 
  summarise(Taxa_rep = sum(consumption_2015)) %>% 
  group_by(Taxa_adj) %>% 
  summarise(baseline_Value=mean(Taxa_rep), baseline_sd=sd(Taxa_rep)) %>% 
  mutate(Element = "2015")
...
ggplot(data= algae_adj, aes(x = Taxa_adj, y = Adj_Value, fill=Element))+
    geom_bar(position = position_dodge(), stat="identity" )+
    geom_errorbar(aes(ymin=Adj_Value-Adj_sd, ymax=Adj_Value+Adj_sd),position = position_dodge(.9),  width=.2)+coord_polar()+
  theme_bw()+
  theme(
  ...
  ggsave("radartest.tiff", dpi=300, device="tiff")
...
+
  ...
  algae_adj %>% 
    ggplot(aes(x = Taxa_adj, y = Savings_mean, group = 1)) +
    geom_point()+
