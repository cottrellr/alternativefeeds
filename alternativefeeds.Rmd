...
(algae_consumption_plot <- 
  ...
  geom_segment(x=0, xend=Inf, y=mean(forage_fish$Value[forage_fish$Year>1979]), yend=mean(forage_fish$Value[forage_fish$Year>1979]), linetype=1)+
  geom_segment(x=0, xend=Inf, y=mean(feed_use$Value[feed_use$Year>1979]), yend=mean(feed_use$Value[feed_use$Year>1979]), colour="grey30", linetype=2)+
  geom_bar(stat="identity", width=.5)+
  geom_errorbar(aes(ymin=Value-sd, ymax=Value+sd), width=.2)+
  scale_fill_manual(values = rev(lacroix_palette("PassionFruit", n=4, "continuous")))+
...
(baseline_2015 <- 
 ...
 (algae_2015 <- 
     ...
     summarise(Taxa_rep_value = sum(consumption_2015)) %>% 
     ungroup()%>% 
     mutate(baseline=baseline_2015$Taxa_rep_value) %>% 
     ...
     geom_bar(stat="identity", colour="grey40", fill="dodgerblue")+
     geom_errorbar(aes(ymin=Mean_savings-SD_savings, ymax=Mean_savings+SD_savings), width=0.2)+
     coord_polar()+
     theme(
     ...
     annotate("text", x=c(4,4,4), y=c(2e+06,4e+6,6e+6), label=c("2", "4", "6"))
 )
