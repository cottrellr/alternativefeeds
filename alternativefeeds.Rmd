...
mean(forage_fish$Value[forage_fish$Year>1980])
...
(supply_demand <- 
  ...
  theme(
    ...
    plot.margin = unit(c(0.5,-.2, 0.2,1.6), "cm"),
...
consumption <- 
  ...
  geom_segment(x=1, xend=Inf, y=mean(forage_fish$Value[forage_fish$Year>1979])*0.8, yend=mean(forage_fish$Value[forage_fish$Year>1979])*0.8, linetype=3)+
  ...
  annotate("text", x=2.5, y=41.5e+6, label="Demand from aquaculture", size=2.8)
...
ggarrange(production_scenarios,
  ggarrange(supply_demand, consumption, widths = c(1,0.7), 
            nrow=1, 
            ncol=2, 
            labels=c("e", ""),
            label.x = 0.15,
  ...
  ggsave("Figure 1 new.tiff", device="tiff", dpi=600, width = 18, height=18, units = "cm")+
  ggsave("Figure 1 new.pdf", device="pdf", dpi=600, width = 18, height=24, units = "cm")
...
length((which((unique(growth_data$Study_id[growth_data$Feed_general=="Algae"]))%%1==0)==TRUE))
length(unique(growth_data$Study_id))
