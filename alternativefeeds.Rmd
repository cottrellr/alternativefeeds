...
fed_aqua$Broad_group[fed_aqua$Species %in% c("Milkfish", "Barramundi(=Giant seaperch)", "Striped bass", "Three-spined stickleback")] <- "Diadromous fishes"
...
"White-spotted spinefoot", "Snappers nei", "Yellowfin seabream",  "Yellowback seabream",  "Meagre",  "Drums nei", "Sobaity seabream", "Red porgy" , "Sharpsnout seabream", "Shi drum",  "Red drum" , "White seabream",   "Okhotsk atka mackerel" , "Porgies, seabreams nei",  "Common snook", "Mojarras(=Silver-biddies) nei", "Percoids nei", "Sargo breams nei" , "Common dentex" ,"Great blue spotted mudskipper", "Snooks(=Robalos) nei", "Common pandora", "Filefishes, leatherjackets nei", "Spinefeet(=Rabbitfishes) nei" , "Common two-banded seabream", "Gobies nei", "Spotted seabass", "Scats", "Snappers, jobfishes nei", "Pargo breams nei", "Goldsilk seabream",  "Russell's snapper", "Marbled spinefoot","Blackspot(=red) seabream" , "Brown meagre",  "Large yellow croaker","Obscure pufferfish", "John's snapper","Papuan black snapper", "Black seabass", "Spotted rose snapper",  "Trumpet emperor", "Two-spot red snapper" , "Streaked spinefoot", "Waigieu seaperch", "Atlantic spadefish", "Japanese seabream"  , "Whitemouth croaker" , "Mi-iuy (brown) croaker", "Tarpon", "Orbicular batfish" , "Sciaenas nei", "Japanese meagre" , "Pink dentex" , "Pufferfishes nei", "Japanese amberjack", "Greater amberjack", "Yellowtail amberjack", "Atlantic cod" , "European seabass", "Flathead grey mullet" , "Squaretail mullet", "Thinlip grey mullet" , "So-iuy mullet",   "Bluespot mullet", "Lebranche mullet", "Mullets nei",  "Red drum", "Japanese seabass", "Large yellow croaker", "Bastard halibut", "Bastard halibuts nei", "Cobia", "Korean rockfish", "Tiger pufferfish", "Amberjacks nei", "Groupers nei", "Greasy grouper", "Hong Kong grouper","Black grouper","Giant grouper", "Spotted coralgrouper","Areolate grouper", "Orange-spotted grouper", "Brown-marbled grouper", "Malabar grouper",  "Humpback grouper", "Groupers, seabasses nei", "Atlantic wolffish", "Spotted wolffish", "Daggertooth pike conger", "Korean rockfish", "Scorpionfishes nei","Cobia", "Japanese jack mackerel",   "White trevally" , "Bigeye trevally"  , "Giant trevally", "Snubnose pompano" , "Silversides(=Sand smelts) nei", "Jacks, crevalles nei", "Bluefish"   , "Crevalle jack" , "Mackerels nei" , "Florida pompano", "Longfin yellowtail", "Golden trevally" ,  "Big-scale sand smelt",  "Malabar trevally", "Atlantic bluefin tuna", "Yellowfin tuna", "Southern bluefin tuna", "Pacific bluefin tuna")] <- "Marine fishes"
...
fed_aqua.global$Year <- as.numeric(fed_aqua.global$Year)
...
Proj_data <- data.frame(Scenario = rep(c("BAU", "CHN", "FAQ"), each=length(seq(2016,2030))*length(levels(fed_aqua.global$Group))), Group=rep(levels(fed_aqua.global$Group), each=length(seq(2016,2030)), 3), Year=rep(seq(2016,2030),3*length(levels(fed_aqua.global$Group))))

#Insert production values from the Props matrix 
Proj_data$Value <- NA
Props_new <- Props[,c(4,5,6)]

for (i in 1:length(levels(Proj_data$Scenario))){
  for (j in 1:length(levels(Proj_data$Group))){
    Proj_data$Value[Proj_data$Year==2030 & Proj_data$Scenario==levels(Proj_data$Scenario)[i] & Proj_data$Group==levels(Proj_data$Group)[j]] <- Props_new[j,i]
  }
}
...
fed_aqua.global$Scenario <-"Data"

fed_aqua_proj <- rbind(fed_aqua.global, Proj_data)
fed_aqua_proj$Scenario <- as.factor(fed_aqua_proj$Scenario)
fed_aqua_proj$Scenario <- factor(fed_aqua_proj$Scenario, levels(fed_aqua_proj$Scenario)[c(3,1,2,4)])

(fed_aqua_proj <- arrange(fed_aqua_proj, Group, Year))


plot(y=fed_aqua_proj$Value, x=fed_aqua_proj$Year, col=fed_aqua_proj$Group, pch=16, cex=0.5)


#assemble GAM data for each group
for (i in 1:length(levels(fed_aqua_proj$Group))){
  df <- fed_aqua_proj[fed_aqua_proj$Group==levels(fed_aqua_proj$Group)[i],]
  model <- gam(Value ~ s(Year), data=df, family = "gaussian"(link = "identity"))
  plotdata <-visreg(model, type = "contrast", plot =F)
  assign(paste("df",i, sep = ""),data.frame(Group = rep(levels(fed_aqua_proj$Group)[i]), x=plotdata$fit[[plotdata$meta$x]], y=plotdata$fit$visregFit+plotdata$fit$Value,lwr=plotdata$fit$visregLwr+plotdata$fit$Value,upper=plotdata$fit$visregUpr+plotdata$fit$Value))
  assign(paste("resids",i, sep = ""),data.frame(Group = rep(levels(fed_aqua_proj$Group)[i]), x=plotdata$res[[plotdata$meta$x]], y=plotdata$res$visregRes+plotdata$res$Value))
}

fit_df <- rbind(df1, df2, df3, df4, df5, df6, df7, df8, df9, df10)
res_df <- rbind(resids1, resids2, resids3, resids4, resids5, resids6, resids7, resids8, resids9, resids10)
...
fit_df$Group <- factor(fit_df$Group,levels(fit_df$Group)[c(1, 9, 10, 8, 2, 5, 7, 6, 3, 4)])
res_df$Group <- factor(res_df$Group,levels(res_df$Group)[c(1, 9, 10, 8, 2, 5, 7, 6, 3, 4)])

ggplot(fit_df, aes(x=x,y=y))+geom_line(aes(colour=Group), linetype="dashed", size=0.3)+
  geom_ribbon(aes(x=x, ymin = lwr, ymax=upper, fill=Group), alpha=0.1)+
  geom_point(data = res_df2, aes(x=x, y=y, colour=Group), size=0.6)+
  
  scale_x_continuous(limits = c(1950,2030), breaks = c(1950,1970,1990,2010, 2030), labels = c(1950, 1970, 1990, 2010, 2030))+
  scale_y_continuous(limits = c(0,19e+06), breaks=c(0, 5e+06, 10e+6, 15e+06), labels = c(0,5,10,15))+
  labs(x=bquote(Year), y=bquote(Production~(T~x10^6)))+
  scale_colour_manual(values=c("#000000" ,"#006449", "#7DBEE7", "#D29E0A", "#BCB984", "#3F9E53", "#1C83BD", "#D4600F", "#CE7179", "#D99F82", "#F0E442"))+
  scale_fill_manual(values=c("#000000" ,"#006449", "#7DBEE7", "#D29E0A", "#BCB984", "#3F9E53", "#1C83BD",  "#D4600F", "#CE7179", "#D99F82", "#F0E442"))+
  theme_bw()+
  theme(panel.grid = element_blank(),
        axis.text = element_text(size=8),
        axis.title = element_text(size=8),
...
ggsave("fig1 - draft.tiff", device = "tiff", dpi=300, width = 10, height = 7, units="cm")
...
feeds$FCR_min <- c(1.6, 1.3, 1.7, 1.5, 1.7, 1.7, 1.5, 1.3, 1.5, 1.6)
feeds$FCR_max <- c(1.7, 1.4, 1.8, 1.5, 1.9, 1.8, 1.7, 1.3, 1.7, 1.7)
feeds$FM_min <- c(0.01, 0.02, 0.02, 0.25, 0.05, 0.15, 0.08, 0.08, 0.05, 0.01)
feeds$FM_max <- c(0.06, 0.03, 0.05, 0.35, 0.1, 0.25, 0.16, 0.12, 0.08, 0.02)
feeds$FO_min <- c(0, 0.01, 0.01, 0.02, 0.008, 0.02, 0.03, 0.06, 0.01, 0)
feeds$FO_max <- c(0.01, 0.014, 0.01, 0.03, 0.01, 0.03, 0.04, 0.08, 0.015, 0)
feeds$FMFO_min <- c(0.02, 0.06, 0.02, 0.24, 0.04, 0.06, 0.08, 0.1, 0.06, 0.01)
feeds$FMFO_max <- c(0.05, 0.11, 0.02, 0.33, 0.09, 0.11, 0.15, 0.2, 0.09, 0.02)
feeds$Fed_min <- c(0.55, 0.80, 0.40, 0.97, 0.55, 0.4, 0.8, 1.00, 0.95, 0.9)
feeds$Fed_max <- c(0.65, 0.85, 0.60, 1.00, 0.65, 0.6, 0.9, 1.00, 0.95, 1.00)

Proj_new <- cbind(Props_new, DATA = fed_aqua.global$Value[fed_aqua.global$Year==2015]) 

Consumption_df <- matrix(data=NA, ncol = 4)
colnames(Consumption_df) <- c("Repeat","Group", "Scenario", "Consumption")

for (i in seq(1:1000)){
  for (s in 1:ncol(Proj_new)){
    for (g in 1:length(feeds$Broad_group)){
      #production and feed requirements from distributions
      fed <- runif(1, feeds$Fed_max[g], feeds$Fed_max[g])
      prod <- Proj_new[g,s]
      fcr <- runif(1, feeds$FCR_min[g], feeds$FCR_max[g])
      #fmfo <- runif(1, feeds$FMFO_min[g], feeds$FMFO_max[g])
      #conversion <-4.44444
      fm <- runif(1, feeds$FM_min[g], feeds$FM_max[g])
      fo <-runif(1, feeds$FM_min[g], feeds$FM_max[g])
      
      #forage fish conversion
      fm_demand <- prod*fcr*fm*fed
      fm_biomass <- fm_demand/0.225
      fo_demand <- prod*fcr*fo*fed
      from_fm_biomass<- fm_biomass*0.05
      gap <- fo_demand-from_fm_biomass
      if (gap > 0 ){
        surplus <- gap/0.05
      } else {
        surplus <- 0
      }
      total_forage <- fm_biomass+surplus
      
      Repeat <- i
      Group <- levels(feeds$Broad_group)[g]
      Scenario <- colnames(Proj_new)[s]
      Consumption <- total_forage#prod*fed*fcr*fmfo*conversion#
      Consumption_df <- rbind(Consumption_df, c(Repeat, Group, Scenario, Consumption))
    }
  }
}
...
Consumption_df <- Consumption_df[2:nrow(Consumption_df),]
Consumption_df$Consumption <- as.numeric(levels(Consumption_df$Consumption))[Consumption_df$Consumption]



Consump_summ <- Consumption_df %>%
  group_by(Scenario, Group) %>%
  summarise(mean=median(Consumption), sd=sd(Consumption))
