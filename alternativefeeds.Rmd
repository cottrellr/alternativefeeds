...
(supply_demand <- 
  bind_rows(forage_fish, feed_use, management) %>% 
    mutate(Element = factor(Element, levels=c("Historical & average supply","Historical & average feed use", "Proposed EBFM supply"))) %>% 
  ...
  theme(
    ...
    legend.position = c(0.37,0.12),
    legend.text = element_text(size=8),
    plot.margin = unit(c(0.5,-.2, 0.2,1.7), "cm"),
...
ggarrange(production_scenarios,
  ggarrange(supply_demand, consumption, widths = c(1,0.6), 
            nrow=1, 
            ncol=2, 
            labels=c("e", ""),
            label.x = 0.18,
  ...
  ggsave("Figure 1 new.tiff", device="tiff", dpi=600, width = 18, height=17, units = "cm")+
  ggsave("Figure 1 new.pdf", device="pdf", dpi=600, width = 18, height=17, units = "cm")
...
#Development-contrained optimal use

```{r}
#same as optimal but only MEDCS and the top producers of fed aquaculture (who are not LDCs i.e. Bangladesh was omitted) have access to novel feeds
...
a_test <- bind_rows(lapply(1:dim(na.exclude(fed_aqua_proj))[1], function(target_row_id){
  ...
  temp_df <- bind_rows(lapply(1:500, function(rep_id){
    ...
    #assign consumption parameters
    b <- runif(n = 1, min = a[["FCR_min"]], max = a[["FCR_max"]])  # assign random fcr from group range
    c <- case_when(target_row[["Country"]] %in% countries_afford ~   (runif(n = 1, min = a[["FM_min"]], max = a[["FM_max"]]))*as.numeric(FM_replace[target_row[["Taxa_adj"]]]),
    ...
    d <- case_when(target_row[["Country"]] %in% countries_afford ~ runif(n = 1, min = a[["FO_min"]], max = a[["FO_max"]]) *as.numeric(FO_replace[target_row[["Taxa_adj"]]]),
...
(optimal_consumption_plot <- 
  ...
  theme(
        ...
        plot.title = element_text(size=7, face = "bold.italic", hjust = 0),
...
(algae_consumption_plot <- 
  ...
  annotate("text", x =0.5, y=31e+6, label="Average supply 1980-2013", hjust=0, size=1.35)+
  annotate("text", x =0.5, y=25e+6, label="Proposed EBFM supply", hjust=0,  size=1.35)+
  annotate("text", x =0.5, y=22.5e+6, label="Average feed use 1980-2013", hjust=0, size=1.35))
...
(baseline_2015 <- 
 ...
 (algae_total <- 
     ...
     theme(
       ...
       panel.grid.major.y =  element_line(colour="grey80", linetype=1, size=0.15),
       ...
       plot.margin = unit(c(t=0.2, r=-0, b=-0.5, l=0), "cm")
     )+
     labs(y=bquote(Forage~fish~savings~(Tx10^6)))+
     guides(fill=FALSE)+
     annotate("text", x=c(4,4,4,4), y=c(2e+06,4e+6,6e+6, 8e+6), label=c("2", "4", "6", "8"), size=1.5)+
  ...
  (bacteria_total <- 
     ...
     mutate(Element = factor(Element, levels = c("2030 BAU", "2030 Rap.Gr", "2030 Cons.Shft"))) %>% 
     ggplot(aes(x=Taxa_adj, y=Mean_savings, fill=Element))+
     geom_bar(stat="identity", colour="grey40",  size=0.1, position=position_dodge())+ #fill="#359DA9",
     ...
     theme(
       ...
       text = element_text(size=7),
       axis.text.x = element_text(angle=c(335, 285, 50, 0, 310, 80, 30 ), vjust=0.5),
       plot.margin = unit(c(t=0.2, r=-0.1, b=-0.5, l=-0.1), "cm")
     )+
     guides(fill=FALSE)+
     annotate("text", x=c(4,4,4,4), y=c(2e+06,4e+6,6e+6, 8e+6), label=c("2", "4", "6", "8"), size=1.5)+
