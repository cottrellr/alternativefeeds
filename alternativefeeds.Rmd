...
# fig1b <- ggplot(data = Now_2030, aes(x=Year, y=Mean, fill=Year))+geom_bar(stat="identity", width = 0.4)+
...
Scenario_consump_df$Scenario <- factor(Scenario_consump_df$Scenario, levels(Scenario_consump_df$Scenario)[c(4,3,5,2,1)])
...
fig1b <- ggplot(data= Scenario_consump_df, aes(x=Scenario, y=Mean))+
  ...
  theme(panel.grid = element_blank(),
        ...
        plot.margin = unit(c(0.2, 0.2, 0.2, -0.4), units = "cm"))+
  scale_y_continuous(limits = c(0, 6e+07), labels = c(0,  20,  40,  60), breaks = c(0,  20e+06,  40e+06, 60e+06))+
  geom_hline(yintercept = mean(forage_fish$Value[forage_fish$Year>1999]), lty=2, colour="firebrick")
  

ggarrange(fig1a, fig1b, labels = c("a", "b"), nrow = 1, ncol = 2, align="h")
