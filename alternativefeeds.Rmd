...
(algae_consumption_plot <- 
  ...
  theme(
        ...
        plot.title = element_text(size=7, face = "bold.italic", hjust = 0.5),
        axis.line = element_line(size=0.25),
        panel.background =  element_blank()
        #plot.margin = unit(c(0.5,.2, 0.2,-1.58), "cm")
        )+
  #guides(fill=F)+
  labs(x="", y=bquote(Forage~fish~consumption~(Tx10^6)), title= "Algae"))
...
(baseline_2015 <- 
 ...
 (algae_total <- 
     ...
     theme(
       panel.grid.major.x =  element_line(color = "grey70", linetype=3, size=0.4),
       panel.grid.major.y =  element_line(colour="grey70", linetype=3, size=0.4),
  ...
  (bacteria_total <- 
     ...
     mutate(Element = factor(Element, levels = c("2030 BAU", "2030 Rap.Gr", "2030 Cons.Shft"))) %>% 
     ggplot(aes(x=Taxa_adj, y=Mean_savings, fill=Element))+
     geom_bar(stat="identity", colour="grey40",  size=0.3, position=position_dodge())+ #fill="#359DA9",
