...
#individual plots and model fitting
```{r}

#Smoothing splines mixed effects models. SME package

#EXAMPLE with steps of
# #FIRST MODEL
# fit <- sme(object = carp_algae$Relative_FCR, tme = carp_algae$Replacement, ind = carp_algae$Study_id, lambda.mu = 0, lambda.v = 0)
# plot(fit,type="model",xlab="Fishmeal replacement",ylab="Relative FCR")
# fit$info #should show zero if converged
# 
# 
# fit <- sme(object = carp_algae$Relative_FCR, tme = carp_algae$Replacement, ind = carp_algae$Study_id, lambda.mu = 1e+7, lambda.v = 1e+7)
# plot(fit,type="model",xlab="Fishmeal replacement",ylab="Relative FCR")
# fit$info #should show zero if converged
# 
# 
# fit <- sme(object = carp_algae$Relative_FCR, tme = carp_algae$Replacement, ind = carp_algae$Study_id,criteria = "AIC")
# plot(fit,type="model",xlab="Fishmeal replacement",ylab="Relative FCR")
# fit$info #should show zero if converged
# 
# 
# fit <- sme(object = carp_algae$Relative_FCR, tme = carp_algae$Replacement, ind = carp_algae$Study_id,criteria = "AIC", knots=myknots)
# plot(fit,type="model",xlab="Fishmeal replacement",ylab="Relative FCR")
# fit$info #should show zero if converged
...
fit_carp_algae <- sme(object = carp_algae$Relative_FCR, tme = carp_algae$Replacement, ind = carp_algae$Study_id,criteria = "AICc")

plot(fit_carp_algae,type="model", xlab="",ylab="Relative FCR", showIndividuals=F,  showConfidenceBands=T, col.meanCurve='#2c7bb6', col=alpha("grey50", 0.5), pch=16, panel.first = c(abline(h=1.05, lty=2, lwd=2, col='black')))
...
#Analysis
myknots <- seq(min(carp_bacteria$Replacement), max(carp_bacteria$Replacement), length.out = 4)[-c(1,4)]
fit_carp_bacteria <- sme(object = carp_bacteria$Relative_FCR, tme = carp_bacteria$Replacement, ind = carp_bacteria$Study_id, criteria = "AICc", knots = myknots)
...
plot(fit_carp_bacteria,type="model",xlab="",ylab="Relative FCR", showIndividuals=F, showConfidenceBands=T, col.meanCurve="#abd9e9", col=alpha("grey50", 0.5), pch=16, panel.first = c(abline(h=1.05, lty=2, lwd=2, col='black')))
...
#Analysis
myknots <- seq(min(carp_yeast$Replacement), max(carp_yeast$Replacement), length.out = 6)[-c(1,6)]
...
plot(fit_carp_yeast,type="model",xlab="",ylab="Relative FCR", showIndividuals=F, showConfidenceBands=T, col.meanCurve='goldenrod', col=alpha("grey50", 0.5), pch=16, panel.first = c(abline(h=1.05, lty=2, lwd=2, col='black')))
...
fit_carp_soy <- sme(object = carp_soy$Relative_FCR, tme = carp_soy$Replacement, ind = carp_soy$Study_id,criteria = "AICc", knots = myknots)
...
plot(fit_carp_soy,type="model",xlab="",ylab="Relative FCR", showIndividuals=F,  xaxs="i", showConfidenceBands=T, col.meanCurve='goldenrod', col=alpha("grey50", 0.5), pch=16, panel.first = c(abline(h=1.05, lty=2, lwd=2, col='black')))
...
plot(fit_carp_insects,type="model",xlab="",ylab="Relative FCR", showIndividuals=F, showConfidenceBands=T, col.meanCurve="#d7191c", pch=16, col=alpha("grey50", 0.5),  panel.first = c(abline(h=1.05, lty=2, lwd=1, col="black")))
...
#create your own length of knots

plot(fit,type="model",xlab="Fishmeal replacement",ylab="Relative FCR", showConfidenceBands=T, col="blue")
...
fit2 <- sme(MTB[MTB$variable==6031,c("y","tme","ind")],
+ criteria == "AIC")
plot(fit2,type="model",xlab="Hour",ylab="Gene Expression")
...
model1 <- gam(Relative_FCR ??? s(Replacement, fx=FALSE, k=-1, bs="cr")+
                factor(Study_id), data=til_alg)
plot(model1)

model1 <- gam(FCR ??? s(Replacement, bs="re"), family="poisson", data=til_alg)
plot(model1)
gam.check(model1)
  
par(mar = c(2, 2, 2, 2))
vis.gam(model1, theta=240)
summary(model1)
anova(model1)

model1 <- gamm(Relative_FCR ~ s(Replacement), data=til_alg,random=list(Study_id=~ 1), family = "poisson" )




plot(model1$fitted.values, resid(model1)) #no serial pattern
plot(resid(model1), til_alg$Replacement) #looks pretty random
hist(resid(model1)) # there is heterogeneity - an even spread of the residual values


ggplot(data=til_alg, aes(x=Replacement, y=Relative_FCR))+geom_point()+geom_smooth(method = "loess")

?geom_smooth()

hist(til_alg$Relative_FCR)
...
carps <- 
  ...
  mutate(Study_id = Study_id %>% 
           as.factor)
# %>% 
  ggplot(data=carps,aes(x=Replacement, y=Relative))+geom_point(alpha=.5)+geom_line(aes(group=Study_id))

library(mgcv)
model1 <- gam(FCR ~ s(Replacement), data=carps)

plot(model1, resid = T, pch = 16)

plot(FCR~Replacement, pch=16, data=carps, ylim= c(0,5))

lines(0:100, predict(model1, newdata=data.frame(Replacement=0:100)), col="Red")

plot(model1, all.terms=F, residuals=T)

hist(carps$Relative_FCR)
...
# Model:
#   -Response: FCR
#   -Fixed effects: Amino_supp,  Replacement
#   -Random effects: Country, Study_id, Life_stage, Feed_specific, Fish_scientific



carps %>% 
  mutate(Predicts = predict(mod.1)) %>% 
  ggplot(aes(x = Replacement, y = FCR)) +
  geom_point(size=3) +
  geom_line(aes(y = predict(mod.1)),size=1) 
