...
```{r}
 ...
 baseline_2015 <- 
  ...
  summarise(Taxa_rep = sum(consumption_2015)) %>% 
  group_by(Taxa_adj) %>% 
  summarise(Algal_Value=mean(Taxa_rep), algal_sd=sd(Taxa_rep)) %>% 
  mutate(Element = "2015") %>% 
  mutate(Baseline_value = baseline_2015$baseline_Value, Baseline_sd = baseline_2015$baseline_sd) %>% 
  mutate(Savings_mean = Baseline_value-Algal_Value) %>% 
  mutate(Savings_lower = (Baseline_value-Baseline_sd)-(Algal_Value+algal_sd), Savings_upper = (Baseline_value+Baseline_sd)-(Algal_Value-algal_sd))
  
  
  algae_2015 %>%
  ggplot(aes(x = Taxa_adj, y = Savings_mean, fill=Element))+
    geom_bar(stat="identity")+
  ...
  algae_2015 %>% 
    ...
    coord_polar() +
    labs(x = "", y = "")+
    annotate("text", x=6, y=3e+06, label="3")+
    annotate("text", x=6, y=2e+06, label="2")+
    annotate("text", x=6, y=1e+06, label="1")
