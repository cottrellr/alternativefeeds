...
(algae_consumption_plot <- 
  ...
  #guides(fill=F)+
  labs(x="", y=bquote(Forage~fish~consumption~(Tx10^6))))
...
(baseline_2015 <- 
 ...
 (algae_total <- 
     ...
     geom_errorbar(aes(ymin=Mean_savings-SD_savings, ymax=Mean_savings+SD_savings), width=0.2, size=0.25, position = position_dodge(0.9))+
     coord_polar()+
     ...
     theme(
       ...
       plot.margin = unit(c(t=0.1, r=-0.1, b=-0.5, l=0), "cm")
     )+
     labs(y=bquote(Forage~fish~savings~(Tx10^6)))+
     guides(fill=FALSE)+
...
#PUT RADARS AND BAR PLOTS TOGETHER
 ...
 use_savings <- ggarrange(
   algae_consumption_plot, bacteria_consumption_plot, yeast_consumption_plot, soy_consumption_plot, insects_consumption_plot,
   algae_total, bacteria_total, yeast_total, soy_total, insects_total,
   labels= letters[1:10],
   widths = c(0.9,0.7,0.7,0.7,0.7,
              1.2,1,1,1,1),
  ...
  ggsave("radar composite.tiff", device="tiff", dpi=600, width = 18, height = 7, units="cm")+
  ggsave("radar composite.pdf", device="pdf", dpi=600, width = 18, height = 7, units =  "cm")

use_savings_legend <- 
  ggarrange(use_savings, scenario_legend, 
          nrow=2,
          ncol=1,
          heights = c(1,0.1))+
  ggsave("Figure 4 - Global use and savings.tiff", device = "tiff", dpi=600, width = 18, height = 8, units = "cm")+
  ggsave("Figure 4 - Global use and savings.pdf", device = "pdf", dpi=600, width = 18, height = 8, units = "cm")



(Figure4 <- 
  annotate_figure(use_savings_legend,
                  top = text_grob(label = paste("            Algae                                    ", "Bacteria                                  ", "Yeast                                   ", "Soy                                  ", "Insects                     "), hjust = 0.44, size=7, face = "bold"))+
  ggsave("Figure 4 - Global use and savings.tiff", device = "tiff", dpi=600, width = 18, height = 8, units = "cm")+
  ggsave("Figure 4 - Global use and savings.pdf", device = "pdf", dpi=600, width = 18, height = 8, units = "cm"))
