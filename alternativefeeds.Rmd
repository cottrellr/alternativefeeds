...
fed_aqua_proj <- fed_aqua %>% 
  filter(Year==2015) %>%
  group_by(Country, Species, Area, Environment, Year, Value, Broad_group) %>%
...
ggplot(fit_df, aes(x=x,y=y))+geom_line(aes(colour=Broad_group), linetype="dashed", size=0.3)+
...
levels(fit_df$Broad_group)
arrange(fed_aqua, -Year, -Value)
```
...
feeds$FCR_min <- c(1.6, 1.3, 1.7, 1.5, 1.7, 1.7, 1.5, 1.3, 1.5, 1.6, 3.4)
feeds$FCR_max <- c(1.7, 1.4, 1.8, 1.5, 1.9, 1.8, 1.7, 1.3, 1.7, 1.7, 7.6)
feeds$FM_min <- c(0.01, 0.02, 0.02, 0.25, 0.05, 0.15, 0.08, 0.08, 0.05, 0.01, 0.1)
feeds$FM_max <- c(0.06, 0.03, 0.05, 0.35, 0.1, 0.25, 0.16, 0.12, 0.08, 0.02, 0.2)
feeds$FO_min <- c(0, 0.01, 0.01, 0.02, 0.008, 0.02, 0.03, 0.06, 0.01, 0, 0.05)
feeds$FO_max <- c(0.01, 0.014, 0.01, 0.03, 0.01, 0.03, 0.04, 0.08, 0.015, 0,0.05)
...
#calculate fishmeal consumption based ooff 2015 data
