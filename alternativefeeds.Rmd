...
fit_carp_bacteria <- sme(object = carp_bacteria$Relative_FCR, tme = carp_bacteria$Replacement, ind = carp_bacteria$Study_id, criteria = "AIC")#, knots = myknots)
...
#Analysis
myknots <- seq(min(carp_soy$Replacement), max(carp_soy$Replacement), length.out = 6)[-c(1,6)]
fit_carp_soy <- sme(object = carp_soy$Relative_FCR, tme = carp_soy$Replacement, ind = carp_soy$Study_id,criteria = "AICc")
...
plot(fit_carp_soy,type="model",xlab="",ylab="Relative FCR", showIndividuals=F, xlim=c(0,100), xaxs="i", showConfidenceBands=T, col.meanCurve='goldenrod', col=alpha("grey50", 0.5), pch=16, panel.first = c(abline(h=1.05, lty=2, lwd=2, col='black')))
