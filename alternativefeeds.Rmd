...
## Plotting changes in production over time of species groups and species based projections
...
for (i in 1: length(levels(fed_aqua$Broad_group))){
  Props[i,1]<- sum(fed_aqua$Value[fed_aqua$Broad_group==levels(fed_aqua$Broad_group)[i] & fed_aqua$Year==2008])/ sum(prod.aqua2$Value[prod.aqua2$Year==2008])
  Props[i,2] <- 93612000
  Props[i,3]<- c(0.18, 0.05, 0.07, 0.01, 0.02, 0.07,0.02, 0.04,0.09, 0.07, 0.00025258)[i]
...
sum(fed_aqua$Value[fed_aqua$Year==2015])*1.37# FAO suggest 55 million by 2030 - our estimate from Props matrix above (58 million)
sum(Props[,4])
...
data1<-fed_aqua_proj[fed_aqua_proj$Group=="Diadromous fishes",]
model1 <- gam(Value ~ s(Year), data=data1, family = "gaussian"(link = "identity"))

(plotdata<-visreg(model1, type = "contrast", plot = T))


fit_df<-data.frame(x=plotdata$fit[[plotdata$meta$x]], y=plotdata$fit$visregFit+plotdata$fit$Value,lwr=plotdata$fit$visregLwr+plotdata$fit$Value,upper=plotdata$fit$visregUpr+plotdata$fit$Value)

for (i in 1:length(fit_df$y)){
  if(fit_df$y[i] <0){
  fit_df$y[i] <- 0
  }
}

for (i in 1:length(fit_df$lwr)){
  if(fit_df$lwr[i] <0){
  fit_df$lwr[i] <- 0
  }
}

for (i in 1:length(fit_df$upper)){
  if(fit_df$upper[i] <0){
  fit_df$upper[i] <- 0
  }
}
...
res_df <- data.frame(x=plotdata$res[[plotdata$meta$x]], y=plotdata$res$visregRes+plotdata$res$Value)

for (i in 1:length(res_df$y)){
  if(res_df$y[i] <0){
  res_df$y[i] <- 0
  }
}
...
ggplot(fit_df, aes(x=x,y=y))+geom_line()+
  geom_line(aes(x=x, y=lwr), linetype="dotted")+
  geom_line(aes(x=x, y=upper), linetype="dotted")+
  geom_point(data = res_df, aes(x=x, y=y))
